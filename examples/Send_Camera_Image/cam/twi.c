...
void i2c_pinMode(int pin, int mode)
{
...
void twi_init(unsigned char sda, unsigned char scl)
{
  ...
  i2c_pinMode(twi_scl, OUTPUT);

  gpio_set_level(twi_sda, 0);
  gpio_set_level(twi_scl, 0);

  i2c_pinMode(twi_sda, INPUT_PULLUP);
  i2c_pinMode(twi_scl, INPUT_PULLUP);
  twi_setClock(100000);
}

void twi_stop(void)
{
  i2c_pinMode(twi_sda, INPUT);
  i2c_pinMode(twi_scl, INPUT);
}
