...
*/
#ifndef ESP8266_CertStoreBearSSL_CPP
#define ESP8266_CertStoreBearSSL_CPP
...
ESP8266_CertStore::~ESP8266_CertStore() {
  free(_indexName);
  free(_dataName);
}

ESP8266_CertStore::ESP8266_Cert_Info ESP8266_CertStore::_preprocessCert(uint32_t length, uint32_t offset, const void *raw) {
  ESP8266_CertStore::ESP8266_Cert_Info ci;
...
int ESP8266_CertStore::initCertStore(FS &fs, const char *indexFileName, const char *dataFileName) {
  int count = 0;
  uint32_t offset = 0;
  ...
  while (true) {
    ...
    if (data.read((uint8_t *)raw, length) != (int)length) {
      free(raw);
      break;
    }
    ...
    if (fileHeader[0] != '/' || fileHeader[1] != '/') {
      ESP8266_CertStore::ESP8266_Cert_Info ci = _preprocessCert(length, offset, raw);
...
void ESP8266_CertStore::installCertStore(br_x509_minimal_context *ctx) {
  br_x509_minimal_set_dynamic(ctx, (void*)this, findHashedTA, freeHashedTA);
}

const br_x509_trust_anchor *ESP8266_CertStore::findHashedTA(void *ctx, void *hashed_dn, size_t len) {
  ESP8266_CertStore *cs = static_cast<ESP8266_CertStore*>(ctx);
  ESP8266_CertStore::ESP8266_Cert_Info ci;
  ...
  while (index.read((uint8_t *)&ci, sizeof(ci)) == sizeof(ci)) {
    if (!memcmp(ci.sha256, hashed_dn, sizeof(ci.sha256))) {
      ...
      data.close();
      cs->_x509 = new ESP8266_X509_List(der, ci.length);
      free(der);
      if (!cs->_x509) {
...
void ESP8266_CertStore::freeHashedTA(void *ctx, const br_x509_trust_anchor *ta) {
  ESP8266_CertStore *cs = static_cast<ESP8266_CertStore*>(ctx);
  (void) ta; // Unused
  delete cs->_x509;
  cs->_x509 = nullptr;
...
#endif /* ESP8266_CertStoreBearSSL_CPP */