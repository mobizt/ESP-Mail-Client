...
const char *WiFiDrv::getBuild()
{
    WAIT_FOR_SLAVE_SELECT();
    // Send Command
    SpiDrv::sendCmd(GET_BUILD_CMD, PARAM_NUMS_0);
    ...
    memset(fwVersion, 0, WL_FW_VER_LENGTH);

    // Wait for reply
    uint8_t _dataLen = 0;
    if (!SpiDrv::waitResponseCmd(GET_BUILD_CMD, PARAM_NUMS_1, (uint8_t *)fwVersion, &_dataLen))
    {
        WARN("error waitResponse");
    }
    SpiDrv::spiSlaveDeselect();
